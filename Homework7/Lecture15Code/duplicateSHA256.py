		...
		for n in range(0, bv4.length(), 1024):
			...
			for i in range(64):
				...
				t1 = BitVector(intVal=(int(h) + int(ch) + int(sum_e) + int(words[i]) + int(K_bv[i])) & \
									  0xFFFFFFFF, size=64)
				t2 = BitVector(intVal=(int(sum_a) + int(maj)) & 0xFFFFFFFF, size=64)
				h = g
				g = f
				f = e
				e = BitVector(intVal=(int(d) + int(t1)) & 0xFFFFFFFF, size=64)
				d = c
				c = b
				b = a
				a = BitVector(intVal=(int(t1) + int(t2)) & 0xFFFFFFFF, size=64)
			...
			h0 = BitVector(intVal=(int(h0) + int(a)) & 0xFFFFFFFF, size=64)
			h1 = BitVector(intVal=(int(h1) + int(b)) & 0xFFFFFFFF, size=64)
			h2 = BitVector(intVal=(int(h2) + int(c)) & 0xFFFFFFFF, size=64)
			h3 = BitVector(intVal=(int(h3) + int(d)) & 0xFFFFFFFF, size=64)
			h4 = BitVector(intVal=(int(h4) + int(e)) & 0xFFFFFFFF, size=64)
			h5 = BitVector(intVal=(int(h5) + int(f)) & 0xFFFFFFFF, size=64)
			h6 = BitVector(intVal=(int(h6) + int(g)) & 0xFFFFFFFF, size=64)
			h7 = BitVector(intVal=(int(h7) + int(h)) & 0xFFFFFFFF, size=64)
