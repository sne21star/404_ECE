...
sudo iptables -t filter -N myfirewall.rules

# Accept all packets generated locally:
sudo iptables -A myfirewall.rules -p all -i lo -j ACCEPT

# Accept all ICMP packets regardless of source:
sudo iptables -A myfirewall.rules -p icmp --icmp-type any -j ACCEPT
...
sudo iptables -A myfirewall.rules -p 50 -j ACCEPT
sudo iptables -A myfirewall.rules -p 51 -j ACCEPT
sudo iptables -A myfirewall.rules -p udp --dport 500 -j ACCEPT
sudo iptables -A myfirewall.rules -p udp --dport 10000 -j ACCEPT

# The destination port 443 is needed both by VPN and by HTTPS:
sudo iptables -A myfirewall.rules -p tcp --dport 443 -j ACCEPT
...
# address 224.0.0.251.
sudo iptables -A myfirewall.rules -p udp --dport 5353 -d 224.0.0.251 -j ACCEPT

# for the Internet Printing Protocol (IPP):
sudo iptables -A myfirewall.rules -p udp -m udp --dport 631 -j ACCEPT

# Accept all packets that are in the states ESTABLISHED and RELATED (See
# Section 18.11 for packet states):
sudo iptables -A myfirewall.rules -p all -m state --state ESTABLISHED,RELATED -j ACCEPT

# I run SSH server on my laptop.  Accept incoming connection requets:
sudo iptables -A myfirewall.rules -p tcp --destination-port 22 -j ACCEPT
...
sudo iptables -A myfirewall.rules -p tcp --destination-port 80 -j ACCEPT

# Drop all other incoming packets.  Do not send back any ICMP messages for
# the dropped packets:
sudo iptables -A myfirewall.rules -p all -j REJECT --reject-with icmp-host-prohibited

sudo iptables -I INPUT -j myfirewall.rules
sudo iptables -I FORWARD -j myfirewall.rules
...
sudo iptables -A myfirewall.rules -s 209.175.44.100 -j DROP
